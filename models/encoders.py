...
class U2Encoder(nn.Module):
    def __init__(self,in_ch=3, latent_dim=cfg.latent_dim):
        super(U2Encoder,self).__init__()
        ...
        self.stage6 = RSU4F(512,256,latent_dim)
...
class U2Decoder(nn.Module):
    def __init__(self,out_ch=3, latent_dim=cfg.latent_dim):
        super(U2Decoder,self).__init__()
        ...
        self.channel_back = REBNCONV(latent_dim, 512)
...
class feature_compress(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.linear3(x)
        return nn.Sigmoid()(x.squeeze(dim=2))
...
if __name__ == "__main__":
    tst = torch.ones((2,3,512,512))
    e1 = Encoder("mobilenet")
    d1 = Decoder()
    fea_comp = feature_compress(img_size=512)
    ...
    print(f2[0].shape,f2[-1].shape)
    print(fea_comp(f2[-1]).shape)
    print(U2Dec(f2)[0].shape,U2Dec(f2)[-1].shape)
